...
def train(ctx):
    ...
    for fold in range(1, 4 + 1):
        ...
        cprint(" function")
        MODEL_NAME = "model_v41_multiclass"
        ...
        new_weights_file = "model_v41_best.hdf5"
...
def create_model(pretrained_weights=None,
    ...
    layer1 = Conv2D(
        64, 3, padding='same', kernel_initializer='he_normal')(input1)
    ...
    layer3 = Conv2D(
        128, 3, padding='same', kernel_initializer='he_normal')(layer2)
    if batch_normalization:
    ...
    layer5 = Conv2D(
        256, 3, padding='same', kernel_initializer='he_normal')(layer4)
    if batch_normalization:
    ...
    layer7 = Conv2D(
        512, 3, padding='same', kernel_initializer='he_normal')(layer6)
    if batch_normalization:
    ...
    layer9 = Conv2D(
        1024, 1, padding='same', kernel_initializer='he_normal')(layer8)
    if batch_normalization:
    ...
    layer11 = Conv2D(
        512, 3, padding='same', kernel_initializer='he_normal')(layer10)
    if batch_normalization:
    ...
    layer13 = Conv2D(
        256, 3, padding='same', kernel_initializer='he_normal')(layer12)
    if batch_normalization:
    ...
    layer15 = Conv2D(
        128, 3, padding='same', kernel_initializer='he_normal')(layer14)
    if batch_normalization:
    ...
    layer17 = Conv2D(
        64, 3, padding='same', kernel_initializer='he_normal')(layer16)
    if batch_normalization:
...
def evaluate(ctx):
    ...
    for (fold, epoch) in zip(fold_list, model_epoch_list):
        ...
        trained_weights_file = f"model_v41_best.hdf5"
        trained_weights_file = os.path.join(weights_dir, trained_weights_file)
