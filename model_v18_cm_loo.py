...
def train(ctx):
    for loo in range(1, 16+1):
        ...
        MODEL_NAME = 'model_v18_multiclass'
        ...
        INPUT_SIZE = (256, 256, 3 + 3) # 3 comes from the 1-pass segments
        TARGET_SIZE = (256, 256)
        NUM_CLASSES = 3
...
def preprocess_segment_input(img):
    ...
    for channel in range(img_boolean.shape[2]):
        ...
        blob_size = np.count_nonzero(img_boolean_each_channel)
        img[img_boolean_each_channel, channel] = blob_size
    # normalize the values of image in 3 channels to be in range [0, 1] with image size
    img[:, :, 0:(img_boolean.shape[2]+1)] /= img.shape[0] * img.shape[1]
    return img
...
def preprocess_images_in_batch(img_batch, segment_batch, img_color_model):
    processed_input_list = []
    image_segment_iterator = zip(img_batch, segment_batch)
    for (img, segment) in image_segment_iterator:
        processed_img = preprocess_image_input(img, img_color_model)
        processed_segment = preprocess_segment_input(segment)
        processed_input = np.concatenate((processed_img, processed_segment), axis=-1)
...
def get_train_data(batch_size,
    ...
    image_flow = image_datagen.flow_from_directory(
        ...
        seed=seed)
    segment_flow = segment_datagen.flow_from_directory(
        train_path,
        classes=[segment_folder],
        class_mode=None,
        color_mode=image_color_mode,
        target_size=target_size,
        batch_size=batch_size,
    ...
    mask_flow = mask_datagen.flow_from_directory(
        ...
        seed=seed)
    image_mask_pair_flow = zip(image_flow, segment_flow, mask_flow)
...
def get_test_data(test_path, image_folder='images', segment_folder='segments', target_size=(256, 256), image_color='rgb', seed=1):
    ...
    segment_flow = segment_datagen.flow_from_directory(
        ...
        shuffle=False,
        seed=seed)
    return zip(test_flow, segment_flow), test_flow.filenames


def train_generator(image_mask_pair_flow, image_color_model):
    for (img_batch, segment_batch, mask_batch) in image_mask_pair_flow:
        processed_img_array = preprocess_images_in_batch(img_batch, segment_batch, image_color_model)
...
def test_generator(test_flow, image_color_model):
    for (img_batch, segment_batch) in test_flow:
        processed_img_array = preprocess_images_in_batch(img_batch, segment_batch, image_color_model)
...
def create(experiment_name_replacement):
    # experiment_name_replacement = "eye_v3-model_v17_multiclass-softmax-cce-lw_1_0.01-hsv-p1-loo_{}-lr_1e_2-bn"
    ...
    segment_filenames = next(os.walk(eye_v3_segments_dir))[2]
    ...
    for (idx, filename) in enumerate(filenames, 0):
        ...
        train_segments_file_list = copy.deepcopy(segment_filenames)
        del train_segments_file_list[idx]
        validation_segments_file_list = segment_filenames[idx]
        ...
        for img in train_segments_file_list:
            move_image_file(eye_v3_segments_dir, train_segments_path, img)
        ...
        src = os.path.join(eye_v3_segments_dir, validation_segments_file_list)
        dest = os.path.join(validation_segments_path, validation_segments_file_list)
        shutil.copy(src, dest)
