...
def predict():
    ...
    for (experiment_name, epoch) in experiment_epoch_pairs:
        weight = f"{epoch:08d}"
...
def get_images_from_batch(
    save_path, results, filenames, weights_name, target_size=(256, 256), num_class=3
):
    ...
    for i, segment in enumerate(results):
        ...
        # skimage.io.imsave(
        #     os.path.join(save_path, f"{file_name}-{weights_name}-{ol+1}-merged.png"),
        #     visualized_img,
        # )
