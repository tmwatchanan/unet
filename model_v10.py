...
def train(ctx):
    ...
    cprint(" function")
    DATASET_NAME = 'eye_v2'
    MODEL_NAME = 'baseline_v10_multiclass'
...
def create_model(pretrained_weights=None,
    ...
    layer7 = Conv2D(
        32, 1, padding='same', kernel_initializer='he_normal')(layer6)
    if batch_normalization:
    ...
    layer9 = Conv2D(
        24, 3, padding='same', kernel_initializer='he_normal')(layer8)
    if batch_normalization:
    ...
    layer11 = Conv2D(
        12, 3, padding='same', kernel_initializer='he_normal')(layer10)
    if batch_normalization:
    ...
    layer13 = Conv2D(
        6, 3, padding='same', kernel_initializer='he_normal')(layer12)
    if batch_normalization:
...
def test_generator(test_path, target_size=(128, 128), color='rgb'):
    file_list = [
