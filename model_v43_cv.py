...
def train(ctx):
    ...
    for fold in range(1, 4 + 1):
        ...
        MODEL_NAME = "model_v43_multiclass"
        ...
        new_weights_file = "model_v43_best.hdf5"
...
def create_model(pretrained_weights=None,
    ...
    layer1 = Conv2D(
        32, 3, padding='same', kernel_initializer='he_normal')(input1)
    ...
    layer17 = Conv2D(
        32, 3, padding='same', kernel_initializer='he_normal')(layer16)
    if batch_normalization:
...
def evaluate(ctx):
    ...
    for (fold, epoch) in zip(fold_list, model_epoch_list):
        ...
        trained_weights_file = f"model_v43_best.hdf5"
        trained_weights_file = os.path.join(weights_dir, trained_weights_file)
