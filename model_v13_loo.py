...
def get_preprocessing_function(color_model):
    if color_model == 'hsv':
...
def train(ctx):
    for loo in range(1, 16 + 1):
        ...
        preprocessing_function_str = get_preprocessing_function(COLOR_MODEL)
        ...
        data_gen_args = dict(
            ...
            fill_mode='nearest',
            preprocessing_function=globals()[preprocessing_function_str])
        if COLOR_MODEL == 'gray':
        ...
        test_gen = test_generator(
            ...
            color=COLOR_MODEL,
            preprocessing_function=preprocessing_function_str)
...
def bgr_to_rgb_preprocessing(img):
    rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    return rgb


def bgr_to_hsv_preprocessing(img):
    hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
    return hsv


def bgr_to_ycbcr_preprocessing(img):
...
def test_generator(test_path,
                   target_size=(256, 256),
                   color='rgb',
                   preprocessing_function='bgr_to_rgb_preprocessing'):
    file_list = [
    ...
    for file_name in file_list:
        ...
        img = globals()[preprocessing_function](img)
        img = img / 255
...
def test(experiment_name, weight, test_dir_name, batch_normalization):
    ...
    # test the model
    test_gen = test_generator(
        test_set_dir, target_size=TARGET_SIZE, color=COLOR_MODEL)
