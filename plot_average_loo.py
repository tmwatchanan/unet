import os
import click
import pandas as pd
import matplotlib.pyplot as plt
import numpy as np


@click.group()
def cli():
    pass


def moving_average(x, N):
    return np.convolve(x, np.ones((N, )) / N)[(N - 1):]


def draw_graph(epoch_list, x, y, x_label, y_label, title, legend, save_name, is_moving_average):
    fig_acc = plt.figure()
    if is_moving_average:
        x = moving_average(x, 100)
        y = moving_average(y, 100)
    plt.plot(epoch_list, x, 'b')
    plt.plot(epoch_list, y, 'g')
    plt.ylabel(x_label)
    plt.xlabel(y_label)
    plt.grid(color='k', linestyle='-', linewidth=1)
    #  plt.ylim(0, 1.0)
    plt.title(title)
    plt.legend(legend, loc='lower right')
    fig_acc.savefig(save_name, bbox_inches='tight')


@cli.command()
@click.option('--moving_average', 'is_moving_average', is_flag=True)
def plot(is_moving_average):
    LOO = 16
    #  experiment_name_template = "eye_v3-baseline_v12_multiclass-softmax-cce-lw_1_0.01-loo_{0}-lr_1e_2-bn"
    experiment_name_template = "eye_v3-baseline_v12_multiclass-softmax-cce-lw_1_0.01-rgb2hsv-loo_{0}-lr_1e_2-bn"
    if (is_moving_average):
        experiment_name_template += "-moving_average"

    graphs_dir = os.path.join('data', 'comparison')

    history_list = []
    for l in range(LOO):
        experiment_name = experiment_name_template.format(l+1)
        output1_acc_file = os.path.join(graphs_dir,
                                        f"{experiment_name}-output1_acc.png")
        dataset_path = os.path.join('data', experiment_name)
        training_log_file = os.path.join(dataset_path, 'training.csv')
        history_data = pd.read_csv(training_log_file)
        draw_graph(history_data['epoch'], history_data['output1_acc'],
                   history_data['val_output1_acc'], 'Accuracy', 'Epoch',
                   f"{experiment_name} - Output 1 Model Accuracy",
                   ['Train Accuracy', 'Validation Accuracy'], output1_acc_file, is_moving_average)
        history_list.append(history_data)
    epoch_list = [a['epoch'] for a in history_list]
    min_epochs = min(map(len, epoch_list))
    epoch_list = range(min_epochs)

    output1_acc_list = [a['output1_acc'] for a in history_list]
    output1_acc_array = np.transpose(np.array(output1_acc_list))
    avg_output1_acc_list = [np.mean(a) for a in output1_acc_array]

    output1_val_acc_list = [a['val_output1_acc'] for a in history_list]
    output1_val_acc_array = np.transpose(np.array(output1_val_acc_list))
    avg_output1_val_acc_list = [np.mean(a) for a in output1_val_acc_array]

    output1_avg_acc_file = os.path.join(
        graphs_dir,
        experiment_name_template.format(f"avg_1_{LOO}") + '.png'
    )
    draw_graph(epoch_list, avg_output1_acc_list, avg_output1_val_acc_list,
               'Accuracy', 'Epoch',
               f"{experiment_name} - Output 1 Model Average Accuracy",
               ['Train Accuracy', 'Validation Accuracy'], output1_avg_acc_file, is_moving_average)
    print("MAX train = ", np.max(avg_output1_acc_list))
    print("MAX validation = ", np.max(avg_output1_val_acc_list))

    # immediately show plotted graphs
    plt.show()


if __name__ == "__main__":
    cli()
